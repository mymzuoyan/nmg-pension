var UserFuWuPrizeList = function () {
    var grid = "";
    var type = 2;
    var handleList = function (method) {
        ...
        grid.init({
            ...
            dataTable: {
                "ajax": function (params, callback, obj) {
                    ...
                    $.post(window.apppath + "/admin/api/user/fuwu/prizeList?serviceOrgId=" + serviceOrgId,
                        {
                ...
                "columns": [
                    ...
                    {
                        "data": "prizeTime", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    {
                        "data": "winningUnit", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    // 获奖名称
                    {
                        "data": "prizeName", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "无";
                    ...
                    },
                    {
                        "data": "id", render: function (data, type, row, meta) {
                            var html = "";
                            if ($("#pimll")) {
                                html += '<a data="' + window.apppath + '/admin/user/fuwu/prizeDetail?id=' + data + '" class="btn default btn-xs green-stripe info" >查看</a>';
        ...
        $(".table-group-action-add").click(function () {
            ...
            window.parent.addTab("tab_prize_add", "从业人员获奖信息新增", href);
        });

        grid.getTable().on("click", "a.info", function () {
            var href = $(this).attr("data");
            window.parent.addTab("tab_prize_detail", "从业人员获奖信息详情", href);
        });
        ...
        grid.getTable().on("click", "button.del", function () {
            ...
            $(this).confirmation({
                ...
                'onConfirm': function () {
                    ...
                    $.post(window.apppath + "/admin/api/user/fuwu/delPrize", {id: topicId}, function (res) {
                        Metronic.unblockUI(tableContainer);
                        if (res.success) {
                            grid.getDataTable().row(curRow).remove().draw(false);
                        } else {
        ...
        function updateArticles(deleteIds) {
            ...
            $.post(window.apppath + "/admin/api/user/fuwu/delSomePrize", {
                deleteIds: deleteIds
            }, function (res) {
                Metronic.unblockUI(tableContainer);
