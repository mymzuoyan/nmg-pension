var FixedAssetsList = function () {
    ...
    var handleList = function (method) {
        ...
        grid.init({
            ...
            dataTable: {
                "ajax": function (params, callback, obj) {
                    $.post(window.apppath + "/admin/api/power/socialDonation/getFixedAssetsList",
                        {
                ...
                "columns": [
                   ...
                   //资产分类
                    ...
                    {
                        "data": "model", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    //实物图片
                    {
                        "data": "foodPicture", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                    ...
                    //发票
                    {
                        "data": "invoice", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                    return '<img src= '+window.apppath+''+data+'>';
                            } else {
                                return "";
                    ...
                    },
                    {
                        "data": "id", render: function (data, type, row, meta) {
                            var html = "";
                            if ($("#pimll")) {
                                html += '<a data="' + window.apppath + '/admin/api/power/socialDonation/fixedAssetsDetail/' + data + '" class="btn default btn-xs green-stripe info" >查看</a>';
        ...
        $(".table-group-action-add").click(function () {
            ...
            window.parent.addTab("tab_fixed_assets_add", "新增居家固定资产信息", href);
        });

        grid.getTable().on("click", "a.info", function () {
            var href = $(this).attr("data");
            window.parent.addTab("tab_fixed_assets_info", "居家固定资产信息详情", href);
        });
        ...
        grid.getTable().on("click", "button.del", function () {
            ...
            $(this).confirmation({
                ...
                'onConfirm': function () {
                    ...
                    $.post(window.apppath + "/admin/api/elderly/deleteFixedAssets", {id: topicId}, function (res) {
                        Metronic.unblockUI(tableContainer);
                        if (res.success) {
                            grid.getDataTable().row(curRow).remove().draw(false);
                        } else {
        ...
        function updateArticles(deleteIds) {
            ...
            $.post(window.apppath + "/admin/api/elderly/deleteSomeFixedAssets", {
                deleteIds: deleteIds
            }, function (res) {
                Metronic.unblockUI(tableContainer);
