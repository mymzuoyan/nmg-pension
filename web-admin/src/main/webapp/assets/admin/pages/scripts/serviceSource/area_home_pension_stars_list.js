 ...
 */
var AreaHomePensionStarsList = function () {
    var _self = "";
    ...
    return {
        ...
        tableInit: function () {
            ...
            _self.grid.init({
                ...
                dataTable: {
                    "ajax": function (params, callback, obj) {
                        $.post(window.apppath + "/admin/api/institution/areaHomePensionStars/list", {
...

                        {
                           ...
                           /*     var columnHtml = '';
                               ...
                               /!* columnHtml += '<button data-id="' + data + '"  class="btn default btn-xs green-stripe update">修改</button>';*!/
                                columnHtml += '<shiro:hasPermission name="person-institution-manager-reaHomePensionStarsList-detail">' +'<button data="' + data + '"   class="btn default btn-xs blue-stripe info">详情</button>' +
                                    '</shiro:hasPermission>';
            ...
            _self.grid.getTable().on("click", "button.del", function (e) {
                ...
                $(this).confirmation({
                    ...
                    'onConfirm': function () {
                        ...
                        $.post(window.apppath + "/admin/api/institution/org/areaHomePensionStars/delete", {id: Id}, function (res) {
            ...
            _self.grid.getTable().on("click", ".info", function () {
                ...
                window.parent.addTab("tab-serviceOrg-subsidy-apply-info", "社区养老服务站星级评定标准", window.apppath + "/admin/institution/infoAreaHomePensionStars/" + data);
            });

            /*社区养老服务站星级评定标准评估*/
            _self.grid.getTable().on("click", ".assess", function () {
                var data = $(this).attr("data");
                window.parent.addTab("tab-serviceOrg-subsidy-apply-info", "社区养老服务站星级评定标准", window.apppath + "/admin/institution/assessAreaHomePensionStars/" + data);
            });
