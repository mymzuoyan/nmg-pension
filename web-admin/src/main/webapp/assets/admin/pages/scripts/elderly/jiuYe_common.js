var JiuYeCommon = function () {
    ...
    return {
        ...
        notAllowEdit: function () {
            ...
            $("#editElderFrom").find("input[name=cardNo]").prop("disabled", true);
            $("#editElderFrom").find("input:file").prop("disabled", true);
        },
        allowEdit: function () {
            $("#editElderFrom").find("input[name=areaNameDiv]").prop("disabled", false);//找到form表单下的所有input标签并置为可编辑
            $("#editElderFrom").find("input[name=serviceOrgName]").prop("disabled", false);
            $("#editElderFrom").find("input[name=name]").prop("disabled", false);
            $("#editElderFrom").find("input[name=mobile]").prop("disabled", false);
            $("#editElderFrom").find("input[name=buTieMoney]").prop("disabled", false);
            $("#editElderFrom").find("input[name=remarks]").prop("disabled", false);
        ...
        editElder: function () {
            ...
            var areaName = $("#editElderFrom input[name=areaNameDiv]").val();
            if (areaName == "") {
                $("#labelmsg").text('请选择所属区域！').addClass("label-danger");
                return false;
            } else if ($("#editElderFrom input[name=name]").val() == "") {
                $("#labelmsg").text('请输入就业人员姓名！').addClass("label-danger");
                return false;
            } else if ($("#editElderFrom input[name=cardNo]").val() == "") {
                ...
                $btn.button('loading');
                    $.ajax({
                    type: "POST",
                    url: window.apppath + "/admin/api/elderly/updateJiuYe",
