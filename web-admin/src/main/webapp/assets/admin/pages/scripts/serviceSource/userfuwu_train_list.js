var UserFuWuTrainList = function () {
    var grid = "";
    var type = 2;
    var handleList = function (method) {
        ...
        grid.init({
            ...
            dataTable: {
                "ajax": function (params, callback, obj) {
                    ...
                    $.post(window.apppath + "/admin/api/user/fuwu/trainList?serviceOrgId=" + serviceOrgId,
                        {
                ...
                "columns": [
                    ...
                    {
                        "data": "trainingTime", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    {
                        "data": "trainingInstitutions", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    // 培训内容
                    {
                        "data": "trainingContents", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "无";
                    ...
                    },
                    {
                        "data": "id", render: function (data, type, row, meta) {
                            var html = "";
                            if ($("#pimll")) {
                                html += '<a data="' + window.apppath + '/admin/user/fuwu/trainDetail?id=' + data + '" class="btn default btn-xs green-stripe info" >查看</a>';
        ...
        $(".table-group-action-add").click(function () {
            ...
            window.parent.addTab("tab_train_add", "从业人员培训信息新增", href);
        });

        grid.getTable().on("click", "a.info", function () {
            var href = $(this).attr("data");
            window.parent.addTab("tab_train_detail", "从业人员培训信息详情", href);
        });
        ...
        grid.getTable().on("click", "button.del", function () {
            ...
            $(this).confirmation({
                ...
                'onConfirm': function () {
                    ...
                    $.post(window.apppath + "/admin/api/user/fuwu/delTrain", {id: topicId}, function (res) {
                        Metronic.unblockUI(tableContainer);
                        if (res.success) {
                            grid.getDataTable().row(curRow).remove().draw(false);
                        } else {
        ...
        function updateArticles(deleteIds) {
            ...
            $.post(window.apppath + "/admin/api/user/fuwu/delSomeTrain", {
                deleteIds: deleteIds
            }, function (res) {
                Metronic.unblockUI(tableContainer);
