var ElderlyNeedsList = function () {
    ...
    var handleList = function (method) {
        ...
        grid.init({
            ...
            dataTable: {
                "ajax": function (params, callback, obj) {
                    $.post(window.apppath + "/admin/api/org/sojournManage/getElderlyNeedsList",
                        {
                            length: params.length,
                            start: params.start,
                            elderName: $('input[name=elderName]').val(),
                            phone: $('input[name=phone]').val(),
                            demandContent: $('input[name=demandContent]').val(),
                            needsTime:$('input[name=needsTime]').val(),
                            orderField: "id",
                            orderDir: "DESC"
                        }, function (res) {
                            if (res) {
                ...
                "columns": [
                    ...
                    {
                        "data": "demandContent", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    {
                        "data": "needsTime", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    },
                    {
                        "data": "id", render: function (data, type, row, meta) {
                            var html = "";
                            if ($("#pimll")) {
                                html += '<a data="' + window.apppath + '/admin/api/org/sojournManage/elderlyNeedsDetail?id=' + data + '" class="btn default btn-xs green-stripe info" >查看</a>';
        ...
        $(".table-group-action-add").click(function () {
            ...
            window.parent.addTab("tab_community_service_add", "新增旅居养老机构信息", href);
        });

        grid.getTable().on("click", "a.info", function () {
            var href = $(this).attr("data");
            window.parent.addTab("tab_bill_info", "旅居养老机构详情", href);
        });
        ...
        grid.getTable().on("click", "button.del", function () {
            ...
            $(this).confirmation({
                ...
                'onConfirm': function () {
                    ...
                    $.post(window.apppath + "/admin/api/org/sojournManage/deleteElderlyNeeds", {id: topicId}, function (res) {
                        Metronic.unblockUI(tableContainer);
                        if (res.success) {
                            grid.getDataTable().row(curRow).remove().draw(false);
                        } else {
        ...
        function updateArticles(deleteIds) {
            ...
            $.post(window.apppath + "/admin/api/org/sojournManage/deleteSomeNeeds", {
                deleteIds: deleteIds
            }, function (res) {
                Metronic.unblockUI(tableContainer);
