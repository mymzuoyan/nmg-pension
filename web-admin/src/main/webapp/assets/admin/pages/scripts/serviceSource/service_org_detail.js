var ServiceOrgDetail = function () {
    ...
    var type = $("#institutionTypeDictId").val();
    if (type != "" && type != null) {
        for (var i = 0; i < obj1.length; i++) {
            if (obj1[i].value == type) {
                obj1[i].checked = true;
            }
        }
    }
    ...
    // 反显建设类型选中的单选框
    var obj1 = document.getElementsByName("buildType");
    var buildType = $("#buildType").val();
    if (buildType != "" && buildType != null) {
        for (var i = 0; i < obj1.length; i++) {
            if (obj1[i].value == buildType) {
                obj1[i].checked = true;
            }
        }
    }
    ...
    return {
        ...
        submitAdd: function () {
            ...
            var cityOrTown = $("input[name=cityOrTown]:checked").val();
        ...
        /*    else if (institutionTypeDictId == null || institutionTypeDictId == ""){
                $("#labelmsg").text('请选择组织类型').addClass("label-danger");
            }
            else if (buildType == null || buildType == ""){
                $("#labelmsg").text('请选择建设类型').addClass("label-danger");
                return false;
            }
            else if (cityOrTown == null || cityOrTown == ""){
                $("#labelmsg").text('请选择城乡类型').addClass("label-danger");
            }*/
        ...
        submitEdit: function () {
            ...
            if (areaName == "") {
                ...
                $("#labelmsg").text('请输入《民办非企业单位登记证书》 或《营业执照》编号！').addClass("label-danger");
            } else if (!flag2) {
                ...
                if (bedCount != null && bedCount != "" && !ServiceOrgDetail.isBedRoom(bedCount)) {
                    ...
                    $.ajax({
                        ...
                        data: $("#serviceOrgEditForm").serialize() +
                        "&serviceProjectName=" + serviceProjectName +
                        "&areaOrder=" + getAreaOrder(areaName) +
                        "&attach=" + attach,
                        dataType: "json",
                        success: function (res) {
                            $btn.button('reset');
