var SojournManageList = function () {
    var grid = "";
    var type = 2;
    var handleList = function (method) {
        ...
        grid.init({
            ...
            dataTable: {
                "ajax": function (params, callback, obj) {
                    $.post(window.apppath + "/admin/api/org/sojournManage/getSojournManageList",
                        {
                ...
                "columns": [
                    ...
                    {
                        "data": "contacts", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    {
                        "data": "contactInformation", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    },
                    {
                        "data": "id", render: function (data, type, row, meta) {
                            var html = "";
                            if ($("#pimll")) {
                                html += '<a data="' + window.apppath + '/admin/api/org/sojournManage/sojournManageDetail?id=' + data + '" class="btn default btn-xs green-stripe info" >查看</a>';
        ...
        grid.getTable().on("click", "button.del", function () {
            ...
            $(this).confirmation({
                ...
                'onConfirm': function () {
                    ...
                    $.post(window.apppath + "/admin/api/org/sojournManage/deleteSojournManage", {id: topicId}, function (res) {
                        Metronic.unblockUI(tableContainer);
                        if (res.success) {
                            grid.getDataTable().row(curRow).remove().draw(false);
                        } else {
        ...
        function updateArticles(deleteIds) {
            ...
            $.post(window.apppath + "/admin/api/org/sojournManage/deleteSome", {
                deleteIds: deleteIds
            }, function (res) {
                Metronic.unblockUI(tableContainer);
