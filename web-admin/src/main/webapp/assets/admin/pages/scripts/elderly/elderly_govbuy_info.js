 ...
 */
var ElderMsgGovBuyInfo = function () {
    ...
    return {
        ...
        init: function (str) {
            _self = this;
            _self.status = str;
            _self.showHandleList();
        },
        showDoc: function () {
            //获取文书列表
            $.post(window.apppath + "/admin/api/elderly/govbuy/apply/getDocsByApplyId", {
                applyId: applyId
            }, function (res) {
                if (res != null) {
                    var html = "";
                    ...
                    for (var i in res) {
                        ...
                        html += "<tr>" +
                            ...
                            "</td>" +
                            "<td>" +
                            "<a onclick='ElderMsgGovBuyInfo.showDocInfo(" + res[i].id + ")'>" +
                            res[i].typeName +
                            "</a>" +
                            "</td><td>" +
                            res[i].createTime +
                    ...
                    $("#apply_doc_list").html(html);
                    $("#docModal").modal("show");
                }
            }, 'json');
        },
        showHandleList:function () {
            //获取文书列表
            $.post(window.apppath + "/admin/api/elderly/govbuy/apply/getHandlesByApplyId", {
                applyId: applyId
            }, function (res) {
                if (res != null) {
                    var html = "";
                    ...
                    $("#apply_handle_list").html(html);
                }
            }, 'json');
        },
        showDocInfo: function (id) {
            popIframe(window.apppath + "/admin/elderly/govbuy/apply/doc/info/" + id, 1000, 600);
        },
        nextStep: function () {
            var str=["下一步","下一步","","下一步","","下一步","","下一步","下一步","下一步","下一步","下一步","下一步","下一步","下一步"]
            var btn1=["受理","初审","","评估任务","","确定","","公示通知","公示回执","","审核通过","","复审通过","","公示通知"]
            var btn2=["不予受理","补正","","不予批准","","取消","","不予批准","取消","","取消","","取消","","不予批准"]
            window.parent.confirm3(str[parseInt(_self.status)],btn1[parseInt(_self.status)],btn2[parseInt(_self.status)], function () {
                ...
                if(_self.status!=5&&_self.status!=8&&_self.status!=10&&_self.status!=12)
                popIframe(window.apppath + "/admin/elderly/govbuy/apply/doc?applyId=" + $("input[name=id]").val() + "&via=2", 1000, 600);
            });
        },
        nextStep2: function () {
            window.parent.confirm3("审核不通过","街道复审","组织复检", function () {
                popIframe(window.apppath + "/admin/elderly/govbuy/apply/doc?applyId=" + $("input[name=id]").val()+"&via=2", 1000, 600);
            }, function () {
                    popIframe(window.apppath + "/admin/elderly/govbuy/apply/doc?applyId=" + $("input[name=id]").val() + "&via=3", 1000, 600);
            });
        },
        queryElder: function () {
        ...
        saveApply: function (type) {
            ...
            if (_self.checkForm()) {
                ...
                $.ajax({
                    type: "POST",
                    url: window.apppath + "/admin/api/elderly/govbuy/apply/update",
                    data: $("#govbuyApplyForm").serialize(),
                    dataType: "json",
        ...
        checkForm: function () {
            ...
            if ((HOME_TEL.length == 0) && (MOBILE.length == 0)) {
                alert("请输入手机号码或固定号码！");
                return false;
            }
            ...
            //本人（或家属）签字
            var FAMILY_NAME = document.getElementById("familyName");
            if (FAMILY_NAME && !FAMILY_NAME.readOnly) {
                if (FAMILY_NAME.value.trim() == "") {
                    alert("请输入本人（或代理人）！");
                    return false;
                }
            }
            //评估员1
            var WORKER_NAME_1 = document.getElementById("workerName1");
            if (WORKER_NAME_1 && !WORKER_NAME_1.readOnly) {
                if (WORKER_NAME_1.value.trim() == "") {
                    alert("请输入评估员1！");
                    return false;
                }
            }
            //工作证号
            var WORKER_CARD_1 = document.getElementById("workerCard1");
            if (WORKER_CARD_1 && !WORKER_CARD_1.readOnly) {
                if (WORKER_CARD_1.value.trim() == "") {
                    alert("请输入评估员1工作证号！");
                    return false;
                }
            }
            //日期
            var WORKER_DATE_1 = document.getElementById("workerDate1");
            if (WORKER_DATE_1 && !WORKER_DATE_1.readOnly) {
                if (WORKER_DATE_1.value.trim() == "") {
                    alert("请输入评估员1的填写日期！");
                    return false;
                }
            }
            //评估员2
            var WORKER_NAME_2 = document.getElementById("workerName2");
            if (WORKER_NAME_2 && !WORKER_NAME_2.readOnly) {
                if (WORKER_NAME_2.value.trim() == "") {
                    alert("请输入评估员2！");
                    return false;
                }
            }
            //工作证号
            var WORKER_CARD_2 = document.getElementById("workerCard2");
            if (WORKER_CARD_2 && !WORKER_CARD_2.readOnly) {
                if (WORKER_CARD_2.value.trim() == "") {
                    alert("请输入评估员2工作证号！");
                    return false;
                }
            }
            //日期
            var WORKER_DATE_2 = document.getElementById("workerDate2");
            if (WORKER_DATE_2 && !WORKER_DATE_2.readOnly) {
                if (WORKER_DATE_2.value.trim() == "") {
                    alert("请输入评估员2的填写日期！");
                    return false;
                }
            }
...
function getScore1() {
    var score01 = document.getElementById("attr1").value;
    if (score01 == 1) {
...
function getScore2() {
    var score02 = document.getElementById("attr2").value;
    if (score02 == 1) {
...
function getScore3() {
    var score03 = document.getElementById("attr3").value;
    if (score03 == 1) {
...
function getScore4() {
    var score04 = document.getElementById("attr4").value;
    if (score04 == 1) {
...
function getScore5() {
    var score05 = document.getElementById("attr5").value;
    if (score05 == 1) {
...
function getScore6() {
    var score06 = document.getElementById("attr6").value;
    if (score06 == 1) {
...
function getScore7() {
    var score07 = document.getElementById("attr7").value;
    if (score07 == 1) {
...
function getScore8() {
    var score08 = document.getElementById("attr8").value;
    if (score08 == 1) {
...
function getScore9() {
    var score09 = document.getElementById("attr9").value;
    if (score09 == 1) {
