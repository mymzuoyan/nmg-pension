 ...
 */
var ElderlyWalletInfoList = function () {
            ...
            _self.grid.init({
                ...
                dataTable: {
                    "ajax": function (params, callback, obj) {
                        $.post(window.apppath + "/admin/api/task/getAllListForWallet", {
                            length: params.length,
                            start: params.start,
                            number: $("input[name=receiverName]").val(),
                    ...
                    "columns": [
                        ...
                        ,
                        {
                            ...
                            var columnHtml = '';
                            if($("#taskCallManagerAppList").val()=="1"){
                                columnHtml += '<a  data="' + data + '" class="btn default btn-xs green-stripe scan">查看</a>';
                            }

                            /*if (parseInt(row.status) == 1)
                                if ($("input[name=canDelete]").val() == "1") {
                                    columnHtml += '<a data="' + data + '"  data-taskreceive="' + row.id + '" class="btn default btn-xs red-stripe delete">撤销</a>';
                                }
                            if (parseInt(row.status) == 2) {
                                if ($("input[name=canDispath]").val() == "1") {
