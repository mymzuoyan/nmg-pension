...
var CallInfoList = function () {
    ...
    return {
        ...
        getCallList: function () {
            ...
            var callerAddress = "";
            if (_self.areaName != "")
                callerAddress = _self.areaName;
            else
                callerAddress = EographicArea.getPosition2();
            _self.areaName = "";
            $.post(window.apppath + "/admin/call/api/callInfo/list", {
                page: _self.page,
                id: $("#searchDiv input[name=id]").val(),
                ...
                handleContent: $("#searchDiv input[name=content]").val(),
                returnVisit: $("#returnVisit").val(),
                startDateStr: $("#searchDiv input[name=startDate]").val(),
                startTimeStr: $("#searchDiv input[name=startTime]").val(),
                endDateStr: $("#searchDiv input[name=endDate]").val(),
                endTimeStr: $("#searchDiv input[name=endTime]").val(),
                ...
                if (res.success) {
                    ...
                    for (var i in infos) {
                        ...
                        if (infos[i].recording != null && infos[i].recording != "") {
                            if ((infos[i].recording).indexOf(".mp3") >= 0)
                        ...
                        html += [
                            ...
                            '                                                            style="float:left;display:inline;width:142px;height:30px;text-align:center;line-height:2;">客户呼入<br><font',
                            '                                                            style="color:#999;">',
                            infos[i].createTime,
                            '                                                    </font></span><span',
                    ...
                    $("#rowContent .callBackA").click(function () {
                        ...
                        window.parent.confirm2("确认回拨号码："+callerId+"吗？",function () {
                            var extension = window.parent.getExtension();
                            var agentId = window.parent.getAgentId();
                            var callerId = $("#confirm input[name=callerId]").val();;
                            var linkedId =$("#confirm input[name=linkedId]").val();;
