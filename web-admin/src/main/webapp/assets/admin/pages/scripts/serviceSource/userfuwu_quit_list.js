var UserFuWuQuitList = function () {
    ...
    var handleList = function (method) {
        ...
        grid.init({
            ...
            dataTable: {
                "ajax": function (params, callback, obj) {
                    ...
                    $.post(window.apppath + "/admin/api/user/fuwu/quitList?serviceOrgId=" + serviceOrgId,
                        {
                ...
                "columns": [
                    ...
                    {
                        "data": "quitTime", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    {
                        "data": "quitReason", render: function (data, type, row, meta) {
                            if (data == 1) {
                                return "辞退";
                            }
                            if (data == 2) {
                    ...
                    // 离职描述
                    {
                        "data": "quitContext", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "无";
                    ...
                    },
                    {
                        "data": "id", render: function (data, type, row, meta) {
                            var html = "";
                            if ($("#pimll")) {
                                html += '<a data="' + window.apppath + '/admin/user/fuwu/quitDetail?id=' + data + '" class="btn default btn-xs green-stripe info" >查看</a>';
        ...
        $(".table-group-action-add").click(function () {
            ...
            window.parent.addTab("tab_quit_add", "从业人员离职信息新增", href);
        });

        grid.getTable().on("click", "a.info", function () {
            var href = $(this).attr("data");
            window.parent.addTab("tab_quit_detail", "从业人员离职信息详情", href);
        });
        ...
        grid.getTable().on("click", "button.del", function () {
            ...
            $(this).confirmation({
                ...
                'onConfirm': function () {
                    ...
                    $.post(window.apppath + "/admin/api/user/fuwu/delQuit", {id: topicId}, function (res) {
                        Metronic.unblockUI(tableContainer);
                        if (res.success) {
                            grid.getDataTable().row(curRow).remove().draw(false);
                        } else {
        ...
        function updateArticles(deleteIds) {
            ...
            $.post(window.apppath + "/admin/api/user/fuwu/delSomeQuit", {
                deleteIds: deleteIds
            }, function (res) {
                Metronic.unblockUI(tableContainer);
