var ServiceOrgDetail = function () {
    ...
    return {
        ...
        setServiceProjectJG: function (strs) {
            serviceProject = strs;
        },
        ...
        submitEdit: function () {
            ...
            var legalRepresentatives1 = $("#legalRepresentatives1").val();
            var homeNurseBedCount = $("#homeNurseBedCount").val();
            var initiateSubject = $("#initiateSubject").val();
            var taxRegistrationNum = $("#taxRegistrationNum").val();
            var institutionsCategory = $("#institutionsCategory").val();
            var runWay = $("#runWay").val();
            var medicalServiceMode = $("#medicalServiceMode").val();
            var whetherToAcceptRemoteElderly = $("#whetherToAcceptRemoteElderly").val();
            ...
            if (areaName == "") {
                ...
                $("#labelmsg").text('请输入通讯地址！').addClass("label-danger");
            } else if (orgCode == null || orgCode == "") {
                $("#labelmsg").text('请输入组织机构代码！').addClass("label-danger");
            }/*else if (businessLicenseNo==null||businessLicenseNo == "") {
                $("#labelmsg").text('（工商登记号）请输入《民办非企业单位登记证书》 或《营业执照》编号！').addClass("label-danger");
            }*/ else if (beginTime == null || beginTime == '') {
                $("#labelmsg").text('请输入开办时间！').addClass("label-danger");
            }else if (legalRepresentatives1 == null || legalRepresentatives1 == '') {
                $("#labelmsg").text('请输入法定代表人！').addClass("label-danger");
            }else if (!flag2) {
                ...
                $("#labelmsg").text('请选择机构性质！').addClass("label-danger");
            } else if (initiateSubject == null || initiateSubject == '') {
                $("#labelmsg").text('请选择兴办主体！').addClass("label-danger");
            } else if (taxRegistrationNum == null || taxRegistrationNum == '') {
                $("#labelmsg").text('请输入税务登记号！').addClass("label-danger");
            } else if (institutionsCategory == null || institutionsCategory == '') {
                $("#labelmsg").text('请选择养老机构类别！').addClass("label-danger");
            }  else if (runWay == null || runWay == '') {
                $("#labelmsg").text('请选择运营方式！').addClass("label-danger");
            } else if (medicalServiceMode == null || medicalServiceMode == '') {
                $("#labelmsg").text('请选择医疗服务方式！').addClass("label-danger");
            } else if (whetherToAcceptRemoteElderly == null || whetherToAcceptRemoteElderly == '') {
                $("#labelmsg").text('请选择是否接收异地老年人！').addClass("label-danger");
                ...
                if (bedCount != null && bedCount != "" && !ServiceOrgDetail.isBedRoom(bedCount)) {
                    ...
                    $("#labelmsg").text('请输入机构性质！').addClass("label-danger");
                }*/ else if (roomProp == null && roomProp == "") {
                    $("#labelmsg").text('请输入用房属性！').addClass("label-danger");
                }else if (regCapital != null && regCapital != "" && !ServiceOrgDetail.isRegCapital2(regCapital)) {
                    $("#labelmsg").text('请输入正确的注册资金！').addClass("label-danger");
                    ...
                    $.ajax({
                        ...
                        data: $("#serviceOrgEditForm").serialize() + "&serviceProjectName=" + serviceProjectName + "&areaOrder=" + getAreaOrder(areaName) + "&attach=" + attach+"&logoUrl="+touXiang,
                        dataType: "json",
                        success: function (res) {
                            $btn.button('reset');
                            if (res.success) {
            ...
            if (typeFile != null && typeFile != "") {
                $.ajaxFileUpload({
                    ...
                    success: function (data, status) {
                        ...
                        $("#photoImg").css("width", "210px");
                        $("#photoImg").css("height", "210px");
