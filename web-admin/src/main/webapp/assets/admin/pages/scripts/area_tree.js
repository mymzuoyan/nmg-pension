...
var AreaTree = function () {
    ...
    return {
        ...
        init: function (sysRole) {
            ...
            _self.plugins = ["types", "wholerow","search"];
            if (_self.isCheckBox) {
        ...
        getSelectAreaName: function (selectId) { //获取选择节点的id
            var node = $('#areaTree').jstree(true).get_node(selectId);
            return node.text;
        },
        ...
        getAllAreaName: function (selectId) { //获取选择区域的全程，（加上父节点）
            ...
            while (node) {
                ...
                node = $('#areaTree').jstree(true).get_node(node.parent);
            }
