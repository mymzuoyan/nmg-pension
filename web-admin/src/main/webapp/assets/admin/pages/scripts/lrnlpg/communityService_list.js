var CommunityServiceList = function () {
    var grid = "";
    var type = 2;
    var handleList = function (method) {
        ...
        grid.init({
            ...
            dataTable: {
                "ajax": function (params, callback, obj) {
                    $.post(window.apppath + "/admin/api/power/socialWork/getSocialWorkList",
                        {
                            length: params.length,
                            start: params.start,
                            serviceItem: $('input[name=serviceItem]').val(),
                            principal: $('input[name=principal]').val(),
                            serviceStartTime: $('input[name=serviceStartTime]').val(),
                ...
                "columns": [
                    ...
                    {
                        "data": "organizationName", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    {
                        "data": "serviceItem", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    {
                        "data": "principal", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    {
                        "data": "serviceStartTime", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    {
                        "data": "serviceEndTime", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
                    ...
                    {
                        "data": "serviceDiary", render: function (data, type, row, meta) {
                            if (data != "" && data != null) {
                                return data;
                            } else {
                                return "";
...

                    {
                        "data": "id", render: function (data, type, row, meta) {
                            var html = "";
                            if ($("#pimll")) {
                                html += '<a data="' + window.apppath + '/admin/api/power/socialWork/communityServiceDetail?id=' + data + '" class="btn default btn-xs green-stripe info" >查看</a>';
        ...
        $(".table-group-action-add").click(function () {
            ...
            window.parent.addTab("tab_community_service_add", "新增社会服务信息", href);
        });

        grid.getTable().on("click", "a.info", function () {
            var href = $(this).attr("data");
            window.parent.addTab("tab_bill_info", "社会服务信息详情", href);
        });
        ...
        grid.getTable().on("click", "button.del", function () {
            ...
            $(this).confirmation({
                ...
                'onConfirm': function () {
                    ...
                    $.post(window.apppath + "/admin/api/elderly/deleteCommunityService", {id: topicId}, function (res) {
                        Metronic.unblockUI(tableContainer);
                        if (res.success) {
                            grid.getDataTable().row(curRow).remove().draw(false);
                        } else {
        ...
        function updateArticles(deleteIds) {
            ...
            $.post(window.apppath + "/admin/api/power/socialWork/deleteSome", {
                deleteIds: deleteIds
            }, function (res) {
                Metronic.unblockUI(tableContainer);
