...
<tiles:insertDefinition name="tiles.template.admin.layout">
        ...
        <div class="row" style="height: 100% !important;">
            <div class="col-md-12" style="height: 100% !important;">
                ...
                <div style="margin-bottom:10px;text-align: right; z-index: 100;top: 10px; right: 20px;position: absolute;">
                    <%--<button  id="alarmMap" style="color: white;" class="btn btn-sm btn-circle btn-import ">实时警告视图--%>
                    <%--</button>--%>
                    <button id="elderMap" style="color: white;" class="btn btn-sm btn-circle">老人实时位置视图
                    </button>
                </div>
    ...
    <tiles:putAttribute name="page_script">
        ...
        <script type="text/javascript"
                src="${pageContext.request.contextPath }/assets/admin/pages/scripts/nurse/nurse_map.js?v=3.8"></script>
        <script>
            jQuery(document).ready(function () {
                Metronic.init();
                Layout.init();
                NurseMap.init();
                NurseMap.addElderMarker_random("");
                $("#elderMap").click();
                //获取类型数量
                $.post(window.apppath + "/admin/api/nurse/alarm/getAlarmTypeCount", {}, function (res) {
                    for (var j in res) {
                        var all = 0;
                        for (var i in alarmTypeName) {
                            if(!isNaN(res[j][alarmTypeName[i]]))
                            {
                                all += parseInt(res[j][alarmTypeName[i]]);
                            }
                            $("#" + j + "_count_" + i).html(res[j][alarmTypeName[i]]);
                        }
                        $("#" + j + "_count_all").html(all);
